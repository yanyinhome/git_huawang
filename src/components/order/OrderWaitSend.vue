<template>
  <div>
    ...
    <div v-if="OrderList.length==0" style="margin-bottom: 5rem">
      <div class="img_box">
        <img src="../../assets/img/order_none.png" alt="">
      </div>
      <div class="info">你还没有可接订单哦~</div>
      <div class="btn_box">
        <mt-button @click="go('/getorders')">去抢单</mt-button>
      </div>
    </div>
        ...
        <div class="orders_box">
          <div class="item_box" v-for="(item,index) in OrderList" :key="index">
            ...
            <template v-if="item.status!=5">
              ...
              <div class="btn_item_box">
                <div class="btn">
                  <mt-button class="noborder" v-bind:class="btnClass.receive" @click="jiedan(item.id)">确定接单</mt-button>
                </div>
                <div class="btn">
                  <mt-button  class="noborder" v-bind:class="btnClass.cancle" @click="jujue(item.id)">拒绝接单</mt-button>
                </div>
              </div>
            </template>
          </div>
    ...
    methods:{
      ...
      loadBottom() {
        ...
        if(this.pageDate.page==this.pageDate.pages){
          ...
          this.pageDate.page++;
          this.$http({method:"post",url:"/api/order/order",data:Object.assign({},{token:this.userInfo.token,type:"1",flag:'5'},{page:this.pageDate.page})})
            .then(
              res=>{
                console.log(res)
      ...
      //拒绝接单
      jujue(id){
        this.$http({method:"post",url:"/api/order/refuse_order",data:{token:this.userInfo.token,id:id}})
          .then(
            res=>{
              console.log(res)
              if(res.data.code==200){
      ...
      go(path,id){
        this.$router.push({path:path,query:{id:id,type:1}})
      },
      //取消订单事件
      cancle(id){
        //取消订单操作
      ...
      //开始配送
      beginSend(id){
        this.$http({method:"post",url:"/api/order/peisong",data:{token:this.userInfo.token,id:id}})
          .then(
            res=>{
              console.log(res)
              if(res.data.code==200){
      ...
      //确定接单
      jiedan(id){
        this.$http({method:"post",url:"/api/order/sub_order",data:{token:this.userInfo.token,id:id}})
          .then(
            res=>{
              console.log(res)
              if(res.data.code==200){
      ...
      getlist(data,next){
        this.$http({method:"post",url:"/api/order/order",data:Object.assign({},{token:this.userInfo.token,type:"1",flag:'5'},data?data:{})})
          .then(
            res=>{
              console.log(res)
              if(res.data.code==200){
    ...
    .body{
      ...
      .right{
        ...
        .info_img{
          ...
          box-sizing: border-box;
          padding-top: 2.5rem;
          background: url("../../assets/img/wait_order.png") no-repeat top center/80%;
