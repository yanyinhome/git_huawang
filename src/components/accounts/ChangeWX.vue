<template>
<div>
  <ReturnHead head_name="微信信息"></ReturnHead>
  <div class="input_box">
    <div class="item">
      <div class="left">微信账户</div>
      <div class="right">
        <input type="text" v-model="inputData.wechat" placeholder="输入微信账户">
      </div>
    </div>
  </div>
  <div class="up_load">
    ...
    <div class="up_load_box">
      <div @click="clickInput" class="img_box" v-if="wechat_img">
        <img  :src="wechat_img" alt="" >
      </div>
      <label v-if="!wechat_img" class="input_box" for="input">&nbsp;</label>
  ...
  export default {
    ...
    methods:{
      ...
      upload(e){
        ...
        axios.post("http://houtai1.cadhx.com/api/home/doupload",param,{headers:{'Content-Type':'multipart/form-data'}})
          .then(res=>{
            if(res.data.code=="200"){
              this.$toast(res.data.msg)
              this.wechat_img=res.data.data.imgurl
              this.inputData.wechat_img=res.data.data.img_id
            }else {
              this.$toast(res.data.msg)
            }
          })
      ...
      submit(){
        this.$http({method:"post",url:"/api/user/editother",data:Object.assign({},{token:this.userInfo.token},this.inputData,{flag:3})})
          .then(
            res=>{
              console.log(res)
              if(res.data.code==200){
    ...
    created () {
      const  {wechat_img,wechat}=this.userInfo.other;
      console.log(wechat_img,wechat)
      this.inputData={
        wechat:wechat,
        wechat_img:''
      }
